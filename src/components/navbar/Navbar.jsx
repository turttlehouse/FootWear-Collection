...
const Navbar = () => {
    ...
    const navList = (
        <ul className="flex flex-col lg:flex-row lg:space-x-4 text-black font-bold lg:text-lg px-5">
            ...
            {/* Shoes */}
            <li>
                <Link to={'/allproduct'} className="nav-link">Shoes</Link>
            </li>
            ...
            {user?.role !== "admin" && (
                <li className="relative">
                    <Link to={'/cart'} className="nav-link flex items-center">
                        <FaShoppingCart className="text-xl mt-1" /> {/* Display the cart icon */}
                        {cartItems.length > 0 && (
                            <sup className="absolute top-0.5 right-0 -mt-3 -mr-4 bg-blue-500 rounded-full px-2 text-xs text-white">
                                {cartItems.length}
                            </sup>
                        )}
                    </Link>
                ...
                <div className="flex items-center space-x-3">
                    <Link to={'/'}>
                        <div className="flex items-center">
                            <h2 className="text-black text-3xl font-bold italic">Footwear Collection</h2>
                        ...
                        <ul className="flex items-center space-x-4">
                            ...
                            {user?.role === "admin" && (
                                <li className="flex items-center">
                                    ...
                                    <Link to={'/admin-dashboard'} className="nav-link font-bold">{user?.name}</Link>
                                </li>
                            )}
                            ...
                            {dropdownOpen && user && (
                                <div className="absolute top-[42px] right-[20px] flex justify-center">
                                    <button onClick={logout} className=" text-black font-bold">
                                        Logout
                                    </button>
                                </div>
                            )}
